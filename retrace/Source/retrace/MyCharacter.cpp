...
AMyCharacter::AMyCharacter()
{
	...
	CameraBoom->bInheritPitch = false; CameraBoom->bInheritYaw = false; 
	CameraBoom->bInheritRoll = false; CameraBoom->SetUsingAbsoluteRotation(true); 
...
void AMyCharacter::Tick(float DeltaTime) 
{
	...
	if (bIsMoving && FootstepSound) 
	{
		...
		{
			UGameplayStatics::PlaySoundAtLocation(GetWorld(),FootstepSound, GetActorLocation() ); FootstepTimer = 0.0f; 
		}
	}
...
void AMyCharacter::Move(const FInputActionValue & Value) 
{
			...
			const FVector Forward = FRotationMatrix(YawRot).GetUnitAxis(EAxis::X); const FVector Right = FRotationMatrix(YawRot).GetUnitAxis(EAxis::Y); AddMovementInput(Forward, InputVector.Y);
...
{
			if (!KnockDownMontage)
				return;// すでに倒れてたら再生しない 
			if (GetCharacterMovement()->IsFalling() == false && GetCharacterMovement()->MovementMode == MOVE_None) {
				return; 
			} 
			// 移動停止 
			GetCharacterMovement()->DisableMovement(); 
			MonsterEffectManager = Cast<AMonsterEffectManager>( UGameplayStatics::GetActorOfClass( GetWorld(), AMonsterEffectManager::StaticClass()));
...
void AMyCharacter::ApplyDeathCamera(FRotator CameraRotation, float CameraDistance, FVector CameraOffset)
{
	if (!CameraBoom)
		return;
